...
public class TestSerizlizer {
    public final String STRING_VALUE_TEST_MYTEST = "String:value:test:mytest:OperationWaybill:";
    public final String STRING_VALUE_TEST_MYTEST_DOT = "String.value.test.mytest.OperationWaybill.";
    //@Autowired
    //public Jedis jedis;
    @Autowired
    ...
    public RedisStringCommands sync;
    ...
    public final static String str = "{\n" +
            ...
            "\t\"clientCode\": \"colp-qy\"\n" +
            "}";
    private static OperationWaybill operationWaybill = JSON.parseObject(str,OperationWaybill.class);
    public final int SUM = 10000;
    public RedisSerializer jdkSerializationRedisSerializer = new JdkSerializationRedisSerializer();
    public RedisSerializer fastJsonRedisSerializer = new FastJsonRedisSerializer(OperationWaybill.class);
    public RedisSerializer fstSerializer = new FstSerializer(OperationWaybill.class);
    public RedisSerializer fstSerializerStr = new FstSerializer(String.class);
    public RedisSerializer fstJsonSerializer = new FstJsonSerializer(String.class);
    public RedisSerializer kryoSerializer = new KryoSerializer(OperationWaybill.class);
    public RedisSerializer protostuffSerializer = new ProtostuffSerializer(OperationWaybill.class);
    public LZ4Serializer lZ4Serializer = new LZ4Serializer(byte[].class);
    ...
    public void main(String[] args) throws IOException {
        ...
        emptyRedis2();
/*
        testFstJSONStrAndSnappy(operationWaybill);
        printRedisMemory();
    ...
    public void printRedisMemory() throws IOException {
        String s = jedisPool.getResource().info("memory");
    ...
    public void testFstJSonSerial(OperationWaybill operationWaybill) {
        long size = 0;
        ...
        for (int i = 0; i < SUM; i++) {
            ...
            jedisPool.getResource().set(String.valueOf(i).getBytes(), jdkserialize);
        ...
        stopWatch.start();
        for (int i = 0; i < SUM; i++) {
            byte[] bytes = jedisPool.getResource().get(String.valueOf(i).getBytes());
            OperationWaybill operationWaybill1 = (OperationWaybill) fstJsonSerializer.deserialize(bytes);
        }

        stopWatch.stop();
        System.out.println(String.format("FST原生JSON序列化方案(fstJsonSerializer)[查询 & 反序列化%s次]\n耗时：%s ms, 总大小 %s mb",
                SUM, stopWatch.getTotalTimeMillis(), BigDecimal.valueOf(size).divide(BigDecimal.valueOf(1024)).divide(BigDecimal.valueOf(1024))));
    }
    ...
    public void simpleValue() {
        ...
        stopWatch.start();
        for (int i = 0; i < SUM; i++) {
            jedisPool.getResource().set(STRING_VALUE_TEST_MYTEST + i, "");
        }

        stopWatch.stop();
        System.out.println(String.format("命名空间形式 [处理%s次]\n耗时：%s ms, 总大小 %s mb",
                SUM, stopWatch.getTotalTimeMillis(), BigDecimal.valueOf(size).divide(BigDecimal.valueOf(1024)).divide(BigDecimal.valueOf(1024))));
    }

    public void simpleValueDot() {

        long size = 0;
        StopWatch stopWatch = new StopWatch();
        stopWatch.start();
        for (int i = 0; i < SUM; i++) {
            jedisPool.getResource().set(STRING_VALUE_TEST_MYTEST_DOT + i, "");
        }

        stopWatch.stop();
        System.out.println(String.format("\".\"分割形式 [处理%s次]\n耗时：%s ms, 总大小 %s mb",
                SUM, stopWatch.getTotalTimeMillis(), BigDecimal.valueOf(size).divide(BigDecimal.valueOf(1024)).divide(BigDecimal.valueOf(1024))));
    }

    public void sourceJSON(OperationWaybill operationWaybill) {
        Jedis resource = null;
        try{
           resource = jedisPool.getResource();
            String key = STRING_VALUE_TEST_MYTEST+UUID.randomUUID().toString();
            byte[] bytes = JSON.toJSONString(operationWaybill).getBytes();
            resource.set(key.getBytes(), bytes);

            OperationWaybill o = JSON.parseObject(resource.get(key.getBytes()), OperationWaybill.class);
        } finally {
            if (resource!=null) {
                resource.close();
            }
    ...
    public void sourceJSONWithOutRedis() {
            byte[] bytes = JSON.toJSONString(operationWaybill).getBytes();
            OperationWaybill o = JSON.parseObject(bytes, OperationWaybill.class);
    }


    public void emptyRedis2() {
        Jedis resource =null;
        try{
            resource = jedisPool.getResource();

            for (String obj : resource.keys(STRING_VALUE_TEST_MYTEST + "*")) {
                resource.del(obj);
            }
        } finally {
            if (resource!=null) {
    ...
    public void init() {
        ...
        //jedis = new Jedis("127.0.0.1", 16379);
        //权限认证
        jedisPool.getResource().auth("123456");

        System.out.println("连接服务成功");
    }


    public void testFst(OperationWaybill operationWaybill) {
        Jedis resource = null;
        try {
            resource = jedisPool.getResource();
            String key = STRING_VALUE_TEST_MYTEST + UUID.randomUUID().toString();
            byte[] jdkserialize = fstSerializer.serialize(operationWaybill);
            resource.set(key.getBytes(), jdkserialize);
            byte[] bytes = resource.get(key.getBytes());
            OperationWaybill operationWaybill1 = (OperationWaybill) fstSerializer.deserialize(bytes);
        }
        finally {
            if (resource !=null) {
                resource.close();
    ...
    public void testFstWithOutRedis() {
            byte[] jdkserialize = fstSerializer.serialize(operationWaybill);
            OperationWaybill operationWaybill1 = (OperationWaybill) fstSerializer.deserialize(jdkserialize);
    }

    public void testSnappy(OperationWaybill operationWaybill) throws IOException {
        Jedis resource = null;
        try{
            resource = jedisPool.getResource();
            String key = STRING_VALUE_TEST_MYTEST+UUID.randomUUID().toString();
    ...
    public void testSnappyWithOutRedis() throws IOException {
            String jsonStr = JSON.toJSONString(operationWaybill);
            byte[] jdkserialize = Snappy.compress(jsonStr.getBytes("UTF-8"));
            byte[] uncompressed = Snappy.uncompress(jdkserialize);
            OperationWaybill operationWaybill1 = JSON.parseObject(new String(uncompressed), OperationWaybill.class);
    }


    public void testKryo(OperationWaybill operationWaybill) {
        Jedis resource = null;
        try{
            resource = jedisPool.getResource();
            String key = STRING_VALUE_TEST_MYTEST+UUID.randomUUID().toString();
            ...
            resource.set(key.getBytes(), jdkserialize);
            byte[] bytes = resource.get(key.getBytes());
            OperationWaybill operationWaybill1 = (OperationWaybill) kryoSerializer.deserialize(bytes);
        } finally {
            if (resource!=null) {
                resource.close();
            }
