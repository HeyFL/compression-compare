 ...
 */
public class TestFinallySnappySerizlizer {
    ...
    private static RedisSerializer fstSerializer = new FstSerializer(OperationWaybill.class);
    ...
    private static void sourceJSON(OperationWaybill operationWaybill) {
        ...
        stopWatch.start();
        for (int i = 0; i < SUM; i++) {
            OperationWaybill o = JSON.parseObject(jedis.get(String.valueOf(i).getBytes()), OperationWaybill.class);
        }
        stopWatch.stop();
    ...
    private static void testFstWrite(OperationWaybill operationWaybill) {
        long size = 0;
        StopWatch stopWatch = new StopWatch();
        stopWatch.start();
        for (int i = 0; i < SUM; i++) {
    ...
    private static void testFstRead(OperationWaybill operationWaybill) {
        long size = 0;
        StopWatch stopWatch = new StopWatch();
        //======查询=======
        stopWatch.start();
    ...
    private static void testSnappyWithJSONParse(OperationWaybill operationWaybill) throws IOException {
        long size = 0;
        StopWatch stopWatch = new StopWatch();
        stopWatch.start();
        for (int i = 0; i < SUM; i++) {
    ...
    private static void testSnappy(OperationWaybill operationWaybill) throws IOException {
        long size = 0;
        StopWatch stopWatch = new StopWatch();
        stopWatch.start();
        for (int i = 0; i < SUM; i++) {
        ...
        stopWatch.stop();
        System.out.println(String.format("Snappy压缩方案[toJSONString + 序列化 & 保存 %s次]\n耗时：%s ms, 总大小 %s mb",
                SUM, stopWatch.getTotalTimeMillis(), BigDecimal.valueOf(size).divide(BigDecimal.valueOf(1024)).divide(BigDecimal.valueOf(1024))));
        ...
        stopWatch.stop();
        System.out.println(String.format("Snappy压缩方案[查询 & 反序列化 + parseObject%s次]\n耗时：%s ms, 总大小 %s mb",
                SUM, stopWatch.getTotalTimeMillis(), BigDecimal.valueOf(size).divide(BigDecimal.valueOf(1024)).divide(BigDecimal.valueOf(1024))));
    }
